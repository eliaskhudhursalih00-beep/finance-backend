...
describe("Transaction Endpoints", () => {
  ...
  it("should create a transaction successfully", async () => {
    const newTx = { type: "expense", amount: 50.5, category_id: 1, description: "Food" };
    pool.query.mockResolvedValue({ 
      rows: [{ id: 10, user_id: 1, name: "food", ...newTx }] 
    });
  ...
  it("should get transactions for user", async () => {
    pool.query.mockResolvedValue({ rows: [{ id: 1, count: "2" }, { id: 2 }] });
